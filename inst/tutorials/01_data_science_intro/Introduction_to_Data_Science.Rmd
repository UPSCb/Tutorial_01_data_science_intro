...
tutorial:
  id: "org.bnbio.tutorials.01_data_science_intro"
  version: 1.0.0
output:
  learnr::tutorial:
    progressive: true
...
The information we got about the dataset mentions difference in foraging. Furthermore, in the EDA __Is that a bias__ section, we observed that Adelie penguins can co-habit with either Chinstrap or Gentoo, but the latter never do. Now that we know a bit more about the data properties, we could devise and test a number of hypotheses that could explain such a difference, for example:

1. Is there a correlation between body mass and flipper length? Does that differ by species? Is that affected by sex?
...
2. Is there a correlation between bill length and depth? Does that differ by species? Is that affected by sex?
